...
def search(request):
    ...
    if 'keywords' in request.GET:
        ...
        if keywords:
            query_set = query_set.filter(titulo__icontains=keywords)
    if 'cidade' in request.GET:
        cidade = request.GET['cidade']
        if cidade:
            query_set = query_set.filter(cidade__iexact=cidade)
    if 'categoria' in request.GET:
        categoria = request.GET['categoria']
        if categoria:
            query_set = query_set.filter(categoria__iexact=categoria)
    if 'estado' in request.GET:
        estado = request.GET['estado']
        if estado:
            query_set = query_set.filter(estado__iexact=estado)
    if 'preco' in request.GET:
        preco = request.GET['preco']
        if preco:
            query_set = query_set.filter(preco__lte=preco)
    context = {
        'query_set': query_set,
...
def delete_listing(request, pk):
    ...
    print(str(user[0].__dict__))
    user2=user[0]
    novo_favorito = user2.favourites
    print('NOVO FAVORITO')
    print(str(novo_favorito))
    novo_favorito = novo_favorito.replace(str(pk),'')
    print('PASSEI DELETE')
    if request.method=="POST":
        print('ENTREI POST')
        user.update(favourites=novo_favorito)
        listing.delete()
        return redirect('dashboard')